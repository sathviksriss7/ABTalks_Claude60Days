...
Good, syntax checks out. Now let me quickly verify it renders correctly with a headless browser check.Tags balance correctly. Let's move the file to outputs and present it.Built it as a 7-agent pipeline for **executive strategy memos**, scored purely on strategic rigor:

**Seed (runs once):** Planner (outline) → Executor (first draft)

**Real loop (live API call every pass, no fixed round count):** Evaluator → Critic → *stop check* → Improver → Memory Manager → back to Evaluator

**Branch (runs once, off the loop, when stop fires):** Final Reviewer

Stop checks fire in order every round: plateau (Δ < 0.3 for two straight rounds) → threshold (your target score) → hard cap (safety fallback, not the intended ending) — and the exact reason is logged and shown in the final summary.

Everything shown — scores, strengths/weaknesses, critique, memory notes, drafts — is the literal text returned by that round's API call; the Evaluator responds in JSON that gets parsed (with regex fallback for robustness) purely to drive the numeric stop-check logic, never to fabricate a score.

Dashboard includes: an animated SVG loop diagram (with a real return arrow and a separate stop-triggered branch to Final Reviewer), live activity log, current draft, evaluation tab, round-by-round history bars, memory notes, retry/error handling with backoff, and a closing summary with agent performance stats, architecture overview, extension ideas, and copyable follow-up prompts.

To use it: open the file, edit the pre-filled brief/audience/objective/target score/cap, and hit **Start pipeline**.

[Autonomous agent studio](autonomous-agent-studio.html)
